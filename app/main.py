...
async def lifespan(app: FastAPI):
    ...
    logger.info("All components initiallized. Ready to serve requests!")
...
app = FastAPI(
    ...
    description="A production-ready chat API with security, caching, and obsevability",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def rate_limit_handler(request: Request, exc: RateLimitExceeded):
    ...
    # Record rate-limit violation in application metrics
...
async def chat(request: Request, body: ChatRequest):
    ...
    with RequestTimer() as timer:
        ...
        if not is_allowed:
            ...
            raise HTTPException(
                status_code=400,
                detail = 'Your message was blocked bt our security filters!'
            )
...
async def get_metrics():
    ...
    return MetricsResponse(
        ...
        error_rate=f"{summary['error_rate']:.2%}",
        avg_latency_ms=summary["average_latency_ms"],
        cache_hit_rate=f"{summary['cache_hit_rate']:.2%}",
        total_input_tokens=summary["tokens_input"],
        total_output_tokens=summary["tokens_output"],
    )
